...
class CUdpHandler {
public:
    CUdpHandler::CUdpHandler(int which_mcdu, char * remote_ip_address, long remote_base_port, long local_base_port, CTransportUdp * parent);
    ...
    SOCKET getUdpSocket();
    void closeUdpSocket();
    size_t writeToUdp(const void* data, size_t len);
    ///////////////////////////////////////////////////////////////////////////////////////////////
private:
    CTransportUdp *        m_parent;       // the transpoart layer that is using this named pipe
public:
    CTransportUdp *        Transport() { return m_parent; }
...
private:
    #define         CUdpHandler_QUEUE_SIZE   300  // room for 299 elements
    unsigned long   m_Pipe_Queue[CUdpHandler_QUEUE_SIZE];
    unsigned int    m_Pipe_Queue_Next_Write;    // index to write to queue:  0..CUdpHandler_QUEUE_SIZE-1
    unsigned int    m_Pipe_Queue_Next_Read;     // index to read from queue: 0..CUdpHandler_QUEUE_SIZE-1
    //#if WANT_PIPE_STATISTICS
    unsigned int    m_Pipe_Queue_High_Water_Mark;   // statistics
    //#endif //WANT_PIPE_STATISTICS

public:
    unsigned int Pipe_Queue_Count(void) {       // number of elements in the queue = 0..CUdpHandler_QUEUE_SIZE-1
        return (CUdpHandler_QUEUE_SIZE + m_Pipe_Queue_Next_Write - m_Pipe_Queue_Next_Read) % CUdpHandler_QUEUE_SIZE;
    }
    ...
    bool Pipe_Queue_Full(void) {
        unsigned int write_next = (m_Pipe_Queue_Next_Write + 1) % CUdpHandler_QUEUE_SIZE;
        return (write_next == m_Pipe_Queue_Next_Read); //or Pipe_Queue_Count()>=(CUdpHandler_QUEUE_SIZE-1);
    }

    bool Write_Pipe_Queue(unsigned long data) {
        unsigned int write_here = m_Pipe_Queue_Next_Write;
        if (!Pipe_Queue_Full()) {
            m_Pipe_Queue_Next_Write = (m_Pipe_Queue_Next_Write + 1) % CUdpHandler_QUEUE_SIZE;
    ...
    unsigned long Read_Pipe_Queue(void) {
        ...
        if (!Pipe_Queue_Empty()) {
            m_Pipe_Queue_Next_Read = (m_Pipe_Queue_Next_Read + 1) % CUdpHandler_QUEUE_SIZE;
...
public:
    #ifdef __WXMSW__    // part of MCDU Simulator
    CTransportUdp(int which_mcdu/*0,1,2,3*/, char * remote_ip_address, long remote_base_port, long local_base_port);
    #else               // part of Display Interface Manager (DIM) Simulator
    CTransportUdp(int which_mcdu/*0,1,2,3*/, char * s_DI_Host, CTransportMCDU * parent);
    #endif
...
public:
    void SetRxCallback(void * pThis, ONRX_HELPER_FUNCTION function) {
        m_udp_haldler->SetRxCallback(pThis, function);
    }

    //void Set_ReaderFail_Callback(void * pThis, ONREADERFAIL_HELPER_FUNCTION function) {
    //    m_udp_haldler->Set_ReaderFail_Callback(pThis, function);
    //}

    //void Set_WriterFail_Callback(void * pThis, ONWRITERFAIL_HELPER_FUNCTION function) {
    //    m_udp_haldler->Set_WriterFail_Callback(pThis, function);
    //}

    void Zap_Transport_Queue(void) { m_udp_haldler->Zap_Transport_Queue(); }

public:
    void            SetQuittingTime(void)   { m_udp_haldler->SetQuittingTime(); }
    unsigned int    PipeStatus(void)        { return m_udp_haldler->PipeStatus(); }
    bool            Pipe_Queue_Full(void)   { return m_udp_haldler->Pipe_Queue_Full(); }
    bool            Is_Write_Allowed(void)  { return m_udp_haldler->Is_Write_Allowed(); }
    int             MCDU(void)              { return m_udp_haldler->MCDU(); }
...
#endif /* __TRANSPORTUDP_H__ */
// end of transportUdp.h
