...
#pragma warning(disable : 4996) /* don't want warning: 'strcpy' may be unsafe */
                                ...
                                /* don't want warning: 'strncat' may be unsafe */
...
#if 1
 #define     MTERM_HEARTBEAT_TEXT    "\x01\x55"
 #define     MTERM_HEARTBEAT_TEXT_SIZE 2
#else
 #define     MTERM_HEARTBEAT_TEXT    "&@%)!$!$$!"
 #define     MTERM_HEARTBEAT_TEXT_SIZE 10
#endif
...
BEGIN_MESSAGE_MAP(CVTWindow, CFrameWnd)
    ...
    ON_COMMAND(ID_CONTROL_DISABLE_HSD_HEARTBEAT, OnDisableHsdHeartbeat)
    ON_COMMAND(ID_CONTROL_ENABLE_HSD_HEARTBEAT, OnEnableHsdHeartbeat)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
CVTWindow::CVTWindow()
{
...
#ifdef TERATERM32
    strcpy(Temp,GetCommandLine()); //zzzzzzzzzzzzzzzzzzzzz
#else
  ...
  if (ts.mcdusIP[0]) CommSetMcdusIP(ts.mcdusIP);
  ...
  Create(VTClassName, "mTerm", Style, rect, NULL, NULL);
  ...
  ::SetTimer(HVTWin, IdHsdHeartbeatTimer, 1000 * 8, NULL);
  m_heartHsdBeatEnabled = 0; /* heartbeat disabled by default to prevent
                                nasty HSD behaviour vgorine 19.04.2010 */
}
...
void CVTWindow::OnClose()
{
  ...
  if (cv.Ready && (cv.PortType==IdTCPIP) &&
      ...
      ! CloseTT &&
      (::MessageBox(HVTWin,"Disconnect?","mTerm",
...
#ifdef MV_CHANGE
  // HSD heartbeat
  else if (nIDEvent==IdHsdHeartbeatTimer)
  {
        if ( (cv.validHsdFlag != 0 ) && (m_heartHsdBeatEnabled != 0) )
        {
            CommRawOut(&cv, MTERM_HEARTBEAT_TEXT, MTERM_HEARTBEAT_TEXT_SIZE);
            CommSend(&cv);
        }
...
void CVTWindow::OnDisableHsdHeartbeat()
{
    m_heartHsdBeatEnabled = 0;
}
void CVTWindow::OnEnableHsdHeartbeat()
{
    m_heartHsdBeatEnabled = 1;
}
