...
HDDEDATA AcceptData(HDDEDATA ItemHSz, HDDEDATA Data)
{
  ...
  /* (im) 14-Nov-2011 DdeCmpStringHandles(HSZ,HSZ) */
  if (DdeCmpStringHandles((HSZ)ItemHSz, (HSZ)Item) != 0) return 0;
  ...
  HSZ hsz1, HSZ hsz2, HDDEDATA Data, DWORD Data1, DWORD Data2)
{
  ...
  switch (CallType) {
    ...
    case XTYP_ADVDATA:
		/*(im) 14-Nov-2011 AcceptData(HDDEDATA,HDDEDATA) */
...
void SendSync()
{
  ...
  // notify free buffer space to mTerm
