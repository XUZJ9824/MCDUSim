...
#pragma warning(disable : 4996) /* don't want warning: 'stricmp' was declared deprecated */
                                /* don't want warning: 'sscanf' was declared deprecated */

/* TTSET.DLL, setup file routines*/
#include "teraterm.h"
#include "tttypes.h"
#include "..\teraterm\commlib.h"
#include <stdio.h>
#include <string.h>
#include <direct.h>
#include "ttlib.h"

#define Section "mTerm"
...
/*  GetPrivateProfileString(Section,"Version","",
  ...
  /* Title String */
  GetPrivateProfileString(Section,"Title","mTerm",
	...
	if (KeyMap->Map[i]==KeyMap->Map[j])
	{
	  if (ShowWarning)
	  {
	    ...
	    MessageBox(0,TempStr,
	      "mTerm: Error in keyboard setup file",MB_ICONEXCLAMATION);
	  }
	  KeyMap->Map[i] = 0xFFFF;
	}
}
...
void FAR PASCAL ParseParam(PCHAR Param, PTTSet ts, PCHAR DDETopic)
{
  ...
  if (DDETopic != NULL)
    DDETopic[0] = 0;
  i = 0;
  /* the first term shuld be executable filename of mTerm */
...
//#ifdef MV_CHANGE
    else if ( strnicmp(Temp,"/M=",3)==0 ) /* COM port num */
    {
        strncpy(ts->mcdusIP, &Temp[3], 64);
    }
//#endif
