...
#pragma warning(disable : 4996) /* don't want warning: 'strcpy' may be unsafe */
                                ...
                                /* don't want warning: the POSIX name for 'getcwd' is deprecated */
...
BOOL FAR PASCAL GetSetupFname(HWND HWin, WORD FuncId, PTTSet ts)
{
  ...
  switch (FuncId) {
    case GSF_SAVE:
      ofn.lpstrTitle = "mTerm: Save setup";
    ...
    case GSF_RESTORE:
      ...
      ofn.lpstrTitle = "mTerm: Restore setup";
    ...
    case GSF_LOADKEY:
      ofn.Flags = ofn.Flags | OFN_FILEMUSTEXIST;
      ofn.lpstrTitle = "mTerm: Load key map";
      Ok = GetOpenFileName(&ofn);
      if (Ok)
  ...
  (HWND HWin, BOOL Receive, LPLONG Option, PFileVar fv, PCHAR CurDir)
{
  ...
  strcpy(fv->DlgCaption,"mTerm: XMODEM ");
  if (Receive)
