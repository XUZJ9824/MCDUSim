...
#pragma warning(disable : 4996) /* don't want warning: 'strcpy' was declared deprecated */
                                /* don't want warning: 'sprintf' was declared deprecated */
                                /* don't want warning: 'strcat' was declared deprecated */
...
void ZSendFileDat(PFileVar fv, PZVar zv)
{
  ...
  SetDlgItemText(fv->HWin, IDC_PROTOFNAME,
  ...
  {
    zv->CRC =
      UpdateCRC(zv->PktOut[zv->PktOutCount],zv->CRC);
    zv->PktOutCount++;
  }
...
void ZSendDataDat(PFileVar fv, PZVar zv)
{
  ...
  if (zv->Pos >= fv->FileSize)
    b = ZCRCE;
  else if ((zv->WinSize >= 0) &&
	   (zv->Pos - zv->LastPos > zv->WinSize))
    b = ZCRCQ;
  else
    b = ZCRCG;
  ...
  (PFileVar fv, PZVar zv, PComVar cv, PTTSet ts)
{
  ...
  strcpy(fv->DlgCaption,"mTerm: ZMODEM ");
  switch (zv->ZMode) {
    case IdZSend:
  ...
  if (cv->PortType==IdTCPIP)
  {
    ...
    switch (ts->Baud) {
      case IdBaud110: Max = 64; break;
      case IdBaud300: Max = 128; break;
      case IdBaud600:
      case IdBaud1200: Max = 256; break;
      case IdBaud2400: Max = 512; break;
      default:
	Max = 1024;
    }
  }
...
BOOL ZCheckHdr(PFileVar fv, PZVar zv)
{
  ...
  {
    switch (zv->ZState) {
      case Z_RecvInit: ZSendRInit(fv,zv); break;
      case Z_RecvData: ZSendRPOS(fv,zv); break;
    }
  }
...
void ZParseRInit(PFileVar fv, PZVar zv)
{
  ...
  if ((zv->ZState != Z_SendInit) &&
      (zv->ZState != Z_SendEOF)) return;
...
{
  if (zv->ZState != Z_RecvInit) return FALSE;
  zv->ZState = Z_RecvInit2;
  zv->CtlEsc = zv->CtlEsc ||
    ((zv->RxHdr[ZF0] & ESCCTL) != 0);
  return TRUE;
}
...
BOOL ZParseFile(PFileVar fv, PZVar zv)
{
  ...
  if ((zv->ZState != Z_RecvInit) &&
      (zv->ZState != Z_RecvInit2)) return FALSE;
  /* kill timer */
  FTSetTimeOut(fv,0);
  zv->CRRecv = FALSE;
...
BOOL ZWriteData(PFileVar fv, PZVar zv)
{
  ...
  if (zv->ZState != Z_RecvData) return FALSE;
  /* kill timer */
  FTSetTimeOut(fv,0);
...
void ZCheckData(PFileVar fv, PZVar zv)
{
  ...
  /* check CRC */
  if (zv->CRC32 && (zv->CRC3 != 0xDEBB20E3) ||
      (! zv->CRC32 && (zv->CRC != 0))) /* CRC */
  {
    switch (zv->ZState) {
      case Z_RecvInit:
      case Z_RecvInit2:
...
BOOL ZParse(PFileVar fv, PZVar zv, PComVar cv)
{
  ...
  do {
    ...
    if (zv->Sending)
    {
      ...
      {
	c = ZWrite(fv,zv,cv,&(zv->PktOut[zv->PktOutPtr]),zv->PktOutCount);
	zv->PktOutPtr = zv->PktOutPtr + c;
	zv->PktOutCount = zv->PktOutCount - c;
      }
      if (zv->PktOutCount<=0)
	zv->Sending = FALSE;
      if ((zv->ZMode==IdZReceive) && (zv->PktOutCount>0)) return TRUE;
    }
    ...
    if (zv->Sending && (zv->PktOutCount>0)) return TRUE;
  } while (zv->Sending);
